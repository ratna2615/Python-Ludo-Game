...
root.configure(background='#011627')
root.title("LUDO")
...
def board():
    # Splash Screen.
    tkinter.messagebox.showinfo(title=None, message="PRESS OKAY TO START GAME")
    v = 0
    z = 0
...
class YBox:  # Class of yellow box
    ...
    def __init__(self, num=-1, x=0, y=0, x0=0, y0=0, double=False, ):
        ...
        # if one game piece on top of another.
        self.double = double

    # Swaps the position of gamepiece according to the number on dice.
    def swap(self):
...
def main():
    ...
    # constructs the game pieces first time the code is ran.
    if c == 0:
        ...
        # list of co-ordinates of all the game pieces in their initial state
        red = [Box() for i in range(4)]

        # that is equal to their respective home co-ordinates.
        blue = [BBox() for i in range(4)]
        green = [GBox() for i in range(4)]
        yellow = [YBox() for i in range(4)]

        # Populates list of homeboxes, colored boxes, gamepieces and white boxes
        for i in range(2):
            redhome[i].x = (70 + (110 * i))
            redhome[i].y = 70
    ...
    # HERE ALL THE GAME OCCURS
    else:
        # This condition is true when a click is made.
        if c >= 1:
            # Red players turn
            if RED == True and TURN == False:
                ...
                la = "RED"
                # Checks if player can take a turn.
                if (movecheck(red, redhome, redbox, la)) == False:
                    BLUE = True
                    RED = False
                    # clears variable, next players turn
                    clear()
                ...
                if RED == True:
                    for i in range(len(red)):
                        if ((((cx > red[i].x0 + 13) and (cx < red[i].x + 13)) and (
                            ...
                            # If a six occurs and gamepiece is in home
                            ...
                            if rolls[0 + nc] == 6:
                                ...
                                # check if all moves are made. so next players turn.
                                if nc > len(rolls) - 1:
                                    BLUE = True
                                    RED = False
                                    clear()
                        ...
                        if ((((cx > red[i].x0 + 13) and (cx < red[i].x + 13)) and
                             ((cy > red[i].y0 + 14) and (cy < red[i].y + 14))) and
                                ((red[i].x0 > 270) or (red[i].y0 > 270))):
                            print("woila ")
                            bb = (red[i].num + rolls[0 + nc])

                            # Winning condition
                            # prevents moves greater than allowed number
                            if bb > 57:
                                break
                            ...
                            # checks if the gamepiece can be made as a double.
                            doublecheck(red)

                            # checks if game has traversed all the blocks
                            nc = nc + 1
                            if bb == 57:
                                # del red[i]
                                red.remove(red[i])
...
def turn():
    ...
    if RED == True:
        L2 = Label(root, text="   Red's Turn    ", fg='RED', background='#011627', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if BLUE == True:
        L2 = Label(root, text="   Blue's Turn   ", fg='BLUE', background='#011627', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if GREEN == True:
        L2 = Label(root, text="Green's Turn  ", fg='GREEN', background='#011627', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)

    if YELLOW == True:
        L2 = Label(root, text="Yellow's Turn", fg='YELLOW', background='#011627', font=("Arial", 24, "bold"))
        L2.place(x=770, y=50)
...
def roll():
    ...
    if TURN == True:
        ...
        if rollc == 1:
            ...
            L1 = Label(root, text=dice, fg='White', background='#011627', font=("Arial", 24, "bold"))
        ...
        if rollc == 2:
            if dice == 6:
                ...
                L3 = Label(root, text=dice1, fg='White', background='#011627', font=("Arial", 24, "bold"))
        ...
        if rollc == 3:
            if dice1 == 6:
                ...
                L4 = Label(root, text=dice2, fg='White', background='#011627', font=("Arial", 24, "bold"))
...
def clear():
    ...
    del rolls[:]
    TURN = True
    L1 = Label(root, text="        ", fg='White', background='#011627', font=("Arial", 24, "bold"))
    L1.place(x=800, y=200)
    L3 = Label(root, text="        ", fg='White', background='#011627', font=("Arial", 24, "bold"))
    L3.place(x=800, y=250)
    L4 = Label(root, text="        ", fg='White', background='#011627', font=("Arial", 24, "bold"))
    L4.place(x=800, y=300)
    print("cleared")
    turn()
...
def movecheck(r, rh, rb, la):
    ...
    if win == True:
        print("YOU HAVE WON")
        L2 = Label(root, text=(la + "Wins"), fg='White', background='green', font=("Arial", 24, "bold"))
