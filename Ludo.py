...
def OnTokenButtonPress(event):
    ...
    var = canvas.find_overlapping(event.x, event.y, event.x, event.y)[0]


# when the button is released
# kindof a Destructor
def OnTokenButtonRelease():
...
def OnTokenMotion(event):
    ...
    drag_data["x"] = event.x
    drag_data["y"] = event.y
    if 444 <= drag_data["x"] <= 582 and 330 <= drag_data["y"] < 462:
        print('pug gayi')
...
def leftClick():  # Main play function is called on every left click.
    x = root.winfo_pointerx()  # - root.winfo_rootx()  # This formula returns the x,y co-ordinates of the mouse 
    # pointer relative to the board. 
